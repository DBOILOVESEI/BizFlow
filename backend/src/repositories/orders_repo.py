...
def get_analytics_data(days=7):
    try:
        ...
        total_debt = sum(o.debt_amount or 0 for o in orders)
        
        # Tạo dữ liệu biểu đồ
        chart_dict = {(end_date - timedelta(days=i)).strftime('%d-%m'): 0 for i in range(days)}
        for o in orders:
            d = o.created_at.strftime('%d-%m')
            if d in chart_dict: chart_dict[d] += (o.final_amount or 0)
            
        return {
            "total_revenue": total_revenue,
            "total_debt": total_debt,
            "total_orders": len(orders),
            "avg_order_value": int(total_revenue/len(orders)) if orders else 0,
            "chart_data": [{"date": k, "revenue": v} for k, v in reversed(chart_dict.items())],
            "payment_cash": sum((o.final_amount or 0) for o in orders if (o.debt_amount or 0) == 0),
            "payment_debt": total_debt
        }
    except Exception as e:
        print(f"Error analytics: {e}")
        return {"total_revenue": 0, "total_debt": 0, "total_orders": 0, "avg_order_value": 0, "chart_data": [], "payment_cash": 0, "payment_debt": 0}