...
class OrderModel(Base):
    ...
    order_type = Column(Enum('AT_COUNTER', 'PHONE', 'ZALO', 'AI_GENERATED', name="OrderTypeEnum"), nullable=False)
    order_status = Column(Enum('DRAFT', 'CONFIRMED', 'COMPLETED', 'CANCELLED', name="OrderStatusEnum"), nullable=False)
    ...
    payment_method = Column(Enum('CASH', 'DEBT', 'BANK_TRANSFER', 'MIXED', name="PaymentMethodEnum"), nullable=False)
    paid_amount = Column(DECIMAL(15,2), nullable=False)
    debt_amount = Column(DECIMAL(15,2), nullable=False)
    notes = Column(Text, nullable=True)
    created_by = Column(Integer, nullable=False)
    confirmed_by = Column(Integer, nullable=False)
